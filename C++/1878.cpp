...
void solve() {
    ull n, k, x; cin >> n >> k >> x;
    // if lower than the minimum and larger than maximum then we can't obtain it
    // otherwise we will always be able to obtain it
    ull minima = k * (k + 1) / 2;
    ull maxima = (k * (n - k + 1 + n)) / 2;

    // debug(minima);
    // debug(maxima);
    if(x > maxima || x < minima) cout << "NO";
    else cout << "YES";
}
