...
void solve() {
  ...
  map<int, set<int>> score_players;
  for(int i = 0; i < n; i++){
    string s = a[i];
    int cur_play_score = 0;
    for(char c : s) cur_play_score += c == 'o';
    score_players[cur_play_score].insert(i);
  }

  for(auto it = score_players.rbegin(); it != score_players.rend(); it++) {
    for(int player : it->second)  cout << player + 1 << " ";
  }
  cout << endl;
} 
