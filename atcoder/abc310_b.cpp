...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      ...
      set<int> jbutnoti;
      set<int> ibutnotj;
      set_difference(all(c[j]), all(c[i]), inserter(jbutnoti, jbutnoti.end())); // get all the functions that exist in j but not in i
      set_difference(all(c[i]), all(c[j]), inserter(ibutnotj, ibutnotj.end())); // get all the functions that exist in i but not in j

      if (pc[i].first >= pc[j].first && ibutnotj.size() == 0 &&
          (pc[i].first > pc[j].first || jbutnoti.size() > 0)) {
        cout << "Yes" << endl;
        return;
      }
    }
